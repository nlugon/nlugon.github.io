...
{% include figure.liquid path="assets/img/isaacsim/ros2-camera-graph.png" title="Stage Tree" class="img-fluid rounded z-depth-1" zoomable=true %}
<p class="text-muted text-center mt-2">Create an Action Graph to publish sensor data through ROS2. </p>


{% include figure.liquid path="assets/img/isaacsim/ros2-context.png" title="Stage Tree" class="img-fluid rounded z-depth-1" zoomable=true %}
<p class="text-muted text-center mt-2">Under Context, ensure the ROS2 Domain ID is the same as the one set in your ROS2 terminals. </p>
...
Next steps will be to actually control the drone via ROS2 and set waypoints or teleoperate the drone manually to get data for running localization algorithms. And then test out the localization algorithms.

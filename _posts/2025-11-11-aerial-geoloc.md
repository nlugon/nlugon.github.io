...
<p class="text-muted text-center mt-2">3D tracking data generated by Google Earth Engine, providing camera ground truth location. Camera FOV here was originally set to the default 20 degrees, but should be set to a higher value, which would allow for larger coverage at a lower altitude (which makes more sense for a drone).</p>
...
{% include figure.liquid path="assets/img/aerial-geoloc/orb-only.png" title="ORB only" class="img-fluid rounded z-depth-1" zoomable=true %}
<p class="text-muted text-center mt-2">Comparision of metrics with other candidatese.</p>

{% include figure.liquid path="assets/img/aerial-geoloc/orb-only2.png" title="ORB only" class="img-fluid rounded z-depth-1" zoomable=true %}
<p class="text-muted text-center mt-2">Location of solution tile on full satellite imag.</p>
